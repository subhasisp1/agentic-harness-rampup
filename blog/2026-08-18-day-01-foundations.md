...
The underrated part: you can render and *inspect* the exact messages before
anything is sent. Then LCEL composes it all with `|`:
...
- Asked what LCEL stands for, the local 32B model said *"Low-Cost Embedded
  Linux."* (It's the LangChain Expression Language.) Fluent ≠ correct —
  that's tomorrow's argument for structured output.
- Context length is memory: the KV-cache OOM above generalizes well beyond
  Ollama.

**Tomorrow:** Pydantic-validated output, and the first tool call run by
hand — the model asks, I execute, I feed the result back. The harness loop,
unrolled once.
